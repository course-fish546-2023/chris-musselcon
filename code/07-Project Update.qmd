...
```
# Project Description  
[![mytilus reef](https://github.com/course-fish546-2023/chris-musselcon/blob/main/assets/mytilusReef.png?raw=true)]  
...
This is the type of sequence data I have available to work with from NCBI so I have to retrieve it. 
...
This is an interim step to look at your DESeq data and see what is interesting/ make any general inferences.
### PCA Plot
```{r}
...
Our PCA shows strong clustering and warrants further review.
### Heatmap
```{r}
...
Our heatmap shows us the contaminant sequences labeled below the color blocks on the X-axis, and up-regulated (red) or down-regulated (blue) genes and the y-axis. When we go back and match the sequence numbers we see confirmation of the result of the PCA. Now we want to look at how many DEGs are statistically significant.
### Volcano Plots
```{r}
...
Now we have something to work with, so we create a result table for futher work.
```{r}
